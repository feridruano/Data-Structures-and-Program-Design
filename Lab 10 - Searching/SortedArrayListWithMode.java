        ...
        implements SortedListInterface< T >
{
    ...
    public T getMode()
    {
        ...
        int modeIndex = 1;
        // Iterate through ArrayList and set a mode to count frequency
        while ( modeIndex <= this.numberOfEntries )
        {
            boolean same = true;
            int compareIndex = modeIndex + 1;
            // Compare entries with modeIndex object
            while ( same && compareIndex <= this.numberOfEntries )
            {
                if ( this.list[compareIndex].equals( this.list[modeIndex] ) )
                    compareIndex++;
                else
                    same = false;
            }
            ...
            if ( compareIndex - modeIndex > modeCount )
            {
                ...
                mode = this.list[modeIndex];
            }

            // Set new modeIndex
            modeIndex += compareIndex - modeIndex; // Jumps to next mode to count frequency
        }
