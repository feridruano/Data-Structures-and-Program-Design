...
public class KthElement
{
    ...
    T kthItem( T[] a, int k, int first, int last )
    {
        ...
        if ( pivotIndex == k )
            return a[k];
        // K in Smaller partition
        else if ( pivotIndex > k )
            kthItem( a, k, first, pivotIndex - 1 );
        // k in Larger partition
        else
            kthItem( a, k, pivotIndex + 1, last );
    ...
    public static void main( String args[] )
    {
        ...
        for ( int i = 1; i <= trials; i++ )
        {
            ...
            for ( int k = 0; k < arraySize; k++ )
            {
                ...
                {
                    System.out.println(
                            ">>> kthItem found median at index " + k + " with value of " + result + " <<<" );
                }
