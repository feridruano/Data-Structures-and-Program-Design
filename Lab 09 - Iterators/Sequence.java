...
public class Sequence
{
    ...
    private boolean isSubSequenceOf( Sequence other )
    {
        // TODO Project 4 - Done
        boolean result = true;
        // Empty [] 'this' sequence is automatically true

        // Check if 'other' sequence has less entries than 'this' sequence
        // OR 'this' sequence has 1 entry and 'other' sequence is empty with [] entries
        if ( other.sequence.size() < this.sequence.size() || other.sequence.get( 0 ).equals( "" ) )
            result = false;
            // Check if 'this' sequence is not empty - 'size' > 0
        else if ( !this.sequence.isEmpty() )
        {
            ...
            // Iterate through 'this' sequence
            while ( result && iterThis.hasNext() )
            {
                boolean foundMatch = false;
                String itemOfThisList = iterThis.next(); // Get entry from 'this' sequence
                // Iterate through 'other' sequence
                while ( !foundMatch && iterOther.hasNext() )
                {
                    String itemOfOtherList = iterOther.next(); // Get entry from 'other' sequence
                    if ( itemOfOtherList.equals( itemOfThisList ) )
                        foundMatch = true; // Compared Strings are equal
                }
                result = foundMatch;
            }
        }
